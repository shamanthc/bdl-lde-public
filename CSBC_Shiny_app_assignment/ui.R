...
body <- dashboardBody(
  ...
  fluidRow(column(4, selectInput("geo", "Select province or territory", choices = Geography)),
           column(4,  selectInput("business_characteristics", "Select input with option groups", 
                                  selected= "All Industries",
                                  choices = list(Industry = Industries_clean ,
                                                 All_employment_sizes = Employment_size_clean,
                                                 All_ownerships= Majority_Ownership_clean,
                                                 All_visible_minorities = Ownership_minority_clean,
                                                 All_business_activities = Business_activity_clean) 
                                     )),
  ...
  tabItem(tabName = "obstacles_newQ",
          ...
          fluidRow(
            ...
            column(11, box(width=12, plotlyOutput("obstacle4_newQ")))
          )
  ),
