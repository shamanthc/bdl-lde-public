talent_server_newQuarter <- function(input, output, session) {
  ...
  output$talent1_newQ <- renderPlot({ 
    ...
    plot_canada <- Required_dataset_19_plot1 %>% filter(GEO=="Canada")
    Required_dataset_19_plot1<- Required_dataset_19_plot1 %>% filter(GEO==input$geo)
    ...
    ggplot(data = Required_dataset_19_plot1 , 
      ...
      geom_point(data= transform(plot_canada, Obstacles= factor(Obstacles, levels=c("Recruiting skilled employees","Retaining skilled employees",
                                                                                    "Shortage of labour force"))),aes(x = factor(Quarter, level = level_order), y = VALUE ), shape=21, fill="#FE4812",color="#FE4812" ,size=2)+
